...
(async function () {
	...
	const JSON_SOURCE_URL = 'https://raw.githubusercontent.com/kjanat/kjanat/refs/heads/master/.github/saved-replies.json';
	...
	const getExistingReplies = () => {
		...
		return [...items].map((el) => {
			const name =
				el
					.querySelector('.listgroup-item-title span')
					?.innerText?.trim() || '';
			const body =
				el
					.querySelector('.listgroup-item-body span')
					?.innerText?.trim() || '';
			return { name, body };
		});
	};
			...
			if (!exists) {
				if (current === index) {
					const form = await waitFor(
						'form[action="/settings/replies"]'
					);
					const titleInput = form.querySelector(
						'input[name="title"]'
					);
					const bodyArea = form.querySelector(
						'textarea[name="body"]'
					);
					...
					console.log(
						`⏳ Importing [${i + 1}/${replies.length}]: ${r.name}`
					);
